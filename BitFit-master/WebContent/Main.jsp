...
<%  try{
	...
	Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/food", "root","faiz");
...
ResultSet lunchResult = statement.executeQuery("select * from userlunch");
		...
		<div class="col-sm-4">
			...
			while(rb.next()){
				...
				if(rb.getString(2).equals("Body Building")){
					ecount = ecount + Integer.parseInt(rb.getString(3))*8;
				}
				if(rb.getString(2).equals("Gymnastics")){
					ecount = ecount + Integer.parseInt(rb.getString(3))*6;
				}
			}
			...
			%>
			<label>Sitting <span style="color:red;">(won't change the calories)</span></label>
			<input type="text" name="Sitting" value="0">
